...
function collectHints(error: unknown, message: string) {
  ...
  if (errorText.includes("cors") || errorText.includes("cross-origin")) {
    ...
    hints.push(
      "Start app with `npm run dev -- --port <APP_PORT>` and localnet on 127.0.0.1:8899."
    );
  }

  if (
    errorText.includes("networkerror") &&
    errorText.includes("fetch resource")
  ) {
    hints.push(
      "Network request to RPC failed. Check that the RPC is running and reachable."
    );
    hints.push(
      "Use the same local endpoint as UI is using and confirm the test validator is up."
    );
  }

  if (
    errorText.includes("disconnected port") ||
    errorText.includes("service worker")
  ) {
    hints.push(
      "Phantom service worker is temporarily unavailable. Re-open Phantom and reconnect the wallet."
    );
  }
